...
from model_parent_category import HGDR
...
parser.add_argument('--target_ddi', type=float, default=0.005, help='DDI-loss activation threshold')
...
parser.add_argument(
    ...
    type=float,
    default=0.5,
    help='weight of the parent-category embedding in the simple baseline (default: 0.5)'
)
...
def main():
    records_path = os.path.join(args.datadir, 'records_final_4.pkl')
    ...
    EHR_path = os.path.join(args.datadir, 'HG_matrix_4.pkl')
    #
    ...
    for epoch in range(epoch_begin, EPOCH):
        ...
        #'''
        savefile = str(run_dir / ('Epoch_%d_JA_%.4f_DDI_%.4f.model' % (epoch, ja, ddi_rate)))
